...
int getRowNum(int e, int t, int param, int upperorlower, int N){
  if(e<CORNER)
    return 8*((e-1)*N-(e-1)) + t*8 + param*2 + upperorlower + 1;
  else
...
int getColNum(int p_e, int t, int param, int N){
  return 4*((p_e)*N-VS_MIN(0,p_e-1)) + t*4 + param + 1;
}
...
#define F(offset, param) inversetransforms[t+offset-1].param;
...
int cameraPathOptimalL1(VSTransformData* td, VSTransformations* trans){
  ...
  if (td->conf.smoothing>0) {
    ...
    for(int t=0; t<N; t++){
      double z = 1.0+ts[t].zoom/100.0;
      double c = cos(ts[t].alpha);
      double s = sin(ts[t].alpha);
      inversetransforms[t].x = (-ts[t].x*c - ts[t].y*s)/z;
      inversetransforms[t].y = (-ts[t].y*c + ts[t].x*s)/z;
      inversetransforms[t].a = c/z;
      inversetransforms[t].b = s/z;
    }
    ...
    int numrows =  getRowNum(4,N,0,0,N);
    ...
    for(int t=0; t< N; t++){
      for(int p=X; p<=B; p++){
        ...
        glp_set_row_bnds(lp, getRowNum(CORNER,t,0,0,N), GLP_DB, -x2-xBorder, -x2); // Corner
        glp_set_row_bnds(lp, getRowNum(CORNER,t,1,0,N), GLP_DB, -y2-yBorder, -y2); // Corner
        glp_set_row_bnds(lp, getRowNum(CORNER,t,2,0,N), GLP_DB, x2, x2+xBorder); // Corner
        glp_set_row_bnds(lp, getRowNum(CORNER,t,3,0,N), GLP_DB, y2, y2+yBorder); // Corner
      }
    }
    ...
    for(int t=0; t< N; t++){
      ...
      ROW(E1, t, X, 0) SET(E1,0,X,1)  SET(P,0,X,1) SET(P,1,A,-F(1,x)) SET(P,1,B,-F(1,y))  SET(P,0,X,-1);
      //          0 >=  -e1X         + w0X             - f1X w1A            - f1Y w1B      - w1X
      ...
      if(t<N-1) {       // |D^2W|
        ...
        ROW(E2, t, A, 0) SET(E2,0,A, 1) SET(P,0,A,-1) SET(P,1,A, 1) SET(P,1,A,+F(2,a)) SET(P,1,B,-1) SET(P,1,B,-F(2,b)) SET(P,2,A,-F(2,a)) SET(P,2,B,+F(2,b));
        ROW(E2, t, A, 1) SET(E2,0,A,-1) SET(P,0,A,-1) SET(P,1,A, 1) SET(P,1,A,+F(2,a)) SET(P,1,B,-1) SET(P,1,B,-F(2,b)) SET(P,2,A,-F(2,a)) SET(P,2,B,+F(2,b));
        ROW(E2, t, B, 0) SET(E2,0,B, 1) SET(P,0,B,-1) SET(P,1,A, 1) SET(P,1,A,+F(2,b)) SET(P,1,B, 1) SET(P,1,B,+F(2,a)) SET(P,2,A,-F(2,b)) SET(P,2,B,-F(2,a));
        ROW(E2, t, B, 1) SET(E2,0,B,-1) SET(P,0,B,-1) SET(P,1,A, 1) SET(P,1,A,+F(2,b)) SET(P,1,B, 1) SET(P,1,B,+F(2,a)) SET(P,2,A,-F(2,b)) SET(P,2,B,-F(2,a));
      }
      if(t<N-2) {       // |D^3W|
        ...
        ROW(E3, t, A, 0) SET(E3,0,A, 1) SET(P,0,A,+1) SET(P,1,A,-2) SET(P,1,A,-F(1,a)) SET(P,1,B,+2) SET(P,1,B,+F(1,b)) SET(P,2,A,+1) SET(P,2,A,+2*F(2,a)) SET(P,2,B,-1) SET(P,2,B,-2*F(2,b)) SET(P,3,A,-F(3,a)) SET(P,3,B,+F(3,b));
        ROW(E3, t, A, 1) SET(E3,0,A,-1) SET(P,0,A,+1) SET(P,1,A,-2) SET(P,1,A,-F(1,a)) SET(P,1,B,+2) SET(P,1,B,+F(1,b)) SET(P,2,A,+1) SET(P,2,A,+2*F(2,a)) SET(P,2,B,-1) SET(P,2,B,-2*F(2,b)) SET(P,3,A,-F(3,a)) SET(P,3,B,+F(3,b));
        ROW(E3, t, B, 0) SET(E3,0,B, 1) SET(P,0,B,+1) SET(P,1,A,-2) SET(P,1,A,-F(1,b)) SET(P,1,B,-2) SET(P,1,B,-F(1,a)) SET(P,2,A,+1) SET(P,2,A,+2*F(2,b)) SET(P,2,B,+1) SET(P,2,B,+2*F(2,a)) SET(P,3,A,-F(3,b)) SET(P,3,B,-F(3,a));
        ROW(E3, t, B, 1) SET(E3,0,B,-1) SET(P,0,B,+1) SET(P,1,A,-2) SET(P,1,A,-F(1,b)) SET(P,1,B,-2) SET(P,1,B,-F(1,a)) SET(P,2,A,+1) SET(P,2,A,+2*F(2,b)) SET(P,2,B,+1) SET(P,2,B,+2*F(2,a)) SET(P,3,A,-F(3,b)) SET(P,3,B,-F(3,a));
      }

      // inclusion and proximity (corners)
      //    -x2 - xB  <= w0X        - w0A x2          - w0B y2 <= -x2
      ROW(CORNER,t,0,0) SET(P,0,X,1) SET(P,0,A,-x2) SET(P,0,B,-y2);
      //     -y2 - yB <= w0Y        + w0B x2         - w0A y2 <= -y2
      ROW(CORNER,t,1,0) SET(P,0,Y,1) SET(P,0,B,x2) SET(P,0,A,-y2);
      //           x2 <= w0X        + w0A x2         + w0B y2 <= x2 + xB
      ROW(CORNER,t,2,0) SET(P,0,X,1) SET(P,0,A,x2) SET(P,0,B,y2);
      //           y2 <= w0Y        - w0B x2         + w0A y2 <= xB + y2
      ROW(CORNER,t,3,0) SET(P,0,Y,1) SET(P,0,B,-x2) SET(P,0,B,y2);
    }
    ...
    for(int t=0; t< N; t++){
      ...
      double z = a*a + b*b;
      ts[t].zoom = 100.0*(z-1.0);
      ts[t].x = (-a*x+b*y)/z;
      ts[t].y = -(b*x+a*y)/z;
      ts[t].alpha = atan2(a,b);
    }
